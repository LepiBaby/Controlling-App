...
## Übersicht
Manuelle Erfassung und Verwaltung von Umsatz-Transaktionen (Erlöse/Verkäufe). Jede Transaktion wird einer Kategorie aus dem Umsatz-KPI-Modell zugeordnet. Die genauen Tabellenfelder werden in einem späteren Sprint definiert — Pflichtfelder für MVP sind Datum, Betrag und Kategoriepfad.

## User Stories
- Als Nutzer möchte ich eine neue Umsatz-Transaktion mit Datum, Betrag und Kategorie erfassen können, damit alle Erlöse vollständig dokumentiert sind.
...
- Als Nutzer möchte ich die Tabelle nach Datum, Betrag und Kategorie filtern/sortieren können, damit ich schnell finde was ich suche.

## Acceptance Criteria
- [ ] Umsatz-Seite zeigt eine Tabelle aller Umsatz-Transaktionen (neueste zuerst)
...
- [ ] Kategorie-Auswahl: Dropdown zeigt zunächst Ebene-1-Kategorien aus dem Umsatz-KPI-Modell; nach Auswahl erscheint Ebene-2-Dropdown (falls vorhanden); analog für Ebene 3
...
- [ ] Betrag-Anzeige in EUR mit Tausender-Trennzeichen (z.B. 12.500,00 €)
...
|--------|---------|-----|-------------|
| Datum | Ja | Date | Datum der Transaktion |
| Betrag | Ja | Decimal | Umsatzbetrag in EUR |
| Kategorie | Ja | KPI-Hierarchie | Aus Umsatz-KPI-Modell (max. 3 Ebenen) |
| Beschreibung | Nein | Text | Freitext-Notiz |
| *weitere TBD* | | | Werden in späterem Sprint ergänzt |

## Edge Cases
- Kein Umsatz-KPI-Modell vorhanden → Eingabeformular zeigt Hinweis "Bitte zuerst KPI-Modell unter Einstellungen definieren" mit Link
