...
export function useFinanzierungsausgaben(referenceDate?: Date) {
    ...
    async function load() {
      ...
      try {
        ...
        const [katRaw, istRaw, berRaw] = await Promise.all([
          ...
          istParams
            ? fetch(`/api/finanzierungs-planung/ist-tatsaechlich${istParams}`).then(r => r.ok ? r.json() : { data: [] })
            : Promise.resolve({ data: [] }),
          berParams
            ? fetch(`/api/finanzierungs-planung/berechnet${berParams}`).then(r => r.ok ? r.json() : { data: [] })
            : Promise.resolve({ data: [] }),
        ])

        // Phase 2b: load manual values AFTER berechnet persisted future Soll to DB
        const valRaw = await fetch('/api/finanzierungs-planung').then(r => r.ok ? r.json() : [])
        ...
        const l1Ids = new Set(
          finanzId
            ? allKats.filter(k => k.parent_id === finanzId).map(k => k.id)
            : [],
        )
        const filtered = allKats.filter(k =>
          (finanzId !== null && k.parent_id === finanzId) ||
        ...
        setError('Fehler beim Laden der Finanzierungsausgaben.')
      } finally {
        setLoading(false)
      }
    }
  ...
  // For past KW: Ist-Plan = the saved manual plan from finanzierungs_planung
  ...
  const upsertZelle = useCallback(
    async (kategorieId: string, kw: PlanungsWoche, value: number | null): Promise<void> => {
      ...
      try {
        const res = await fetch('/api/finanzierungs-planung', {
          method: 'PUT',
  ...
  const resetAll = useCallback(async (): Promise<void> => {
    ...
    try {
      const params = firstFuture
        ...
        : ''
      const res = await fetch(`/api/finanzierungs-planung${params}`, { method: 'DELETE' })
      if (!res.ok) throw new Error()
    } catch {
      setValues(snapshot)
    ...
    if (vergangenheitswochen.length > 0 || zukunftswochen.length > 0) {
      ...
      fetch(`/api/finanzierungs-planung/berechnet${berParams}`)
