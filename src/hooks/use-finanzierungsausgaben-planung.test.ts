...
import { buildFinanzierungsausgabenMonate, betragCellKey } from './use-finanzierungsausgaben-planung'

// PROJ-90: Reine Logik der langfristigen Finanzierungsausgaben Planung —
// Monatsfenster (Start EXAKT im Startmonat, KEIN Vorlauf, Horizont Spalten,
// Jahresgrenzen) und Schlüssel-Helfer. Spiegelung der PROJ-88-Logiktests.

describe('buildFinanzierungsausgabenMonate', () => {
  it('startet exakt im Startmonat und liefert genau Horizont Spalten (kein Vorlauf)', () => {
    const monate = buildFinanzierungsausgabenMonate(4, 2026, 12) // Start April 2026, Horizont 12
  ...
  it('läuft korrekt über die Jahresgrenze (Start im Dezember)', () => {
    const monate = buildFinanzierungsausgabenMonate(12, 2026, 3) // Dez 2026, Horizont 3
  ...
  it('läuft fortlaufend über mehrere Jahre (langer Horizont)', () => {
    const monate = buildFinanzierungsausgabenMonate(6, 2026, 30) // 30 Monate ab Juni 2026
  ...
  it('liefert mindestens eine Spalte (Fallback bei Horizont 0)', () => {
    const monate = buildFinanzierungsausgabenMonate(4, 2026, 0)
  ...
  it('erzeugt für jeden Monat ein nicht-leeres Label', () => {
    const monate = buildFinanzierungsausgabenMonate(4, 2026, 3)
