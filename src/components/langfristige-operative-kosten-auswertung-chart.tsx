...
// PROJ-98: Gestapeltes Diagramm der Operative-Kosten-Auswertung. Je L1-Gruppe eine
// gestapelte Fläche; die übereinandergestapelten Gruppen ergeben zusammen die
// „Operative Kosten (Gesamt)". Im Wachstums-Modus ist Stapeln sinnlos → dann Linien.
...
export function LangfristigeOperativeKostenAuswertungChart({ model, anzeigemodus }: Props) {
  const { columns, operativ, brutto, loading } = model
  ...
  const chartData = useMemo(() => {
    ...
    // Im Diagramm werden die Kosten als POSITIVE Magnituden dargestellt (Nutzerwunsch
    // 2026-06-24): die Gruppen stapeln nach oben zur positiven Gesamtsumme. Die Tabelle
    // bleibt davon unberührt (dort negativ/rot, konsistent mit PROJ-95).
    return columns.map((col, colIdx) => {
      const point: Record<string, string | number | undefined> = { name: col.label }
      for (const g of gruppen) {
        const mag = Math.abs(g.values[col.key] ?? 0)
        if (anzeigemodus === 'absolut') {
          point[g.id] = mag
        } else if (anzeigemodus === 'prozentual') {
          const basis = bruttoBasis?.[col.key] ?? 0
          point[g.id] = basis === 0 ? undefined : (mag / basis) * 100
        } else {
          const vorMag = colIdx > 0 ? Math.abs(g.values[columns[colIdx - 1].key] ?? 0) : undefined
          const w = calcWachstum(mag, vorMag)
  ...
  const titel = anzeigemodus === 'wachstum' ? 'Operative Kosten — Wachstum je Gruppe' : 'Operative Kosten — Zusammensetzung'
