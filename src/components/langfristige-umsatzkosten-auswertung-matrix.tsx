...
// PROJ-97: Umsatzkosten-Matrix der Langfristigen Planung — drei Kostenarten
// (Produktkosten, Vertriebskosten, Marketingkosten) + Summe „Umsatzkosten (Gesamt)",
// Drill-Down bis Produkt, sticky erste Spalte, drei Ansichtsmodi.
// Aufbau und Stil exakt wie die Rentabilitäts-/Umsatz-Matrix (PROJ-95/96).
...
function flatten(nodes: UkNode[], expandedIds: Set<string>, indent: number, out: FlatRow[]) {
...
export function LangfristigeUmsatzkostenAuswertungMatrix({ model, anzeigemodus }: Props) {
                ...
                return (
                  <tr
                    ...
                    className={[
                      ...
                      isSubtotal ? 'bg-muted border-t-2 border-t-border' : 'hover:bg-muted/20',
                    ].filter(Boolean).join(' ')}
                  >
