...
import { useLangfristigeBestellungKosten } from '@/hooks/use-langfristige-bestellung-kosten'
import type { LangfristigeBestellungKosten } from '@/hooks/use-langfristige-bestellung-kosten'
import { useKpiCategories } from '@/hooks/use-kpi-categories'

// PROJ-86: Bestellkosten-Tabelle der Langfristigen Planung — 1:1 wie kurzfristig
// (PROJ-64), nur versionsgebunden. Kategorien stammen wie kurzfristig aus dem
// globalen ausgaben_kosten-KPI-Modell (Unterkategorien von „Produkt").
...
function toFormState(k: LangfristigeBestellungKosten): FormState {
  return {
    datum: k.datum,
...
export function LangfristigeBestellkostenTabelle({ versionId, bestellungId, readOnly = false }: Props) {
  const { toast } = useToast()
  const { kosten, loading, error, add, update, remove } = useLangfristigeBestellungKosten(versionId, bestellungId)
  ...
  function startEdit(k: LangfristigeBestellungKosten) {
    setEditingId(k.id)
  ...
  async function toggleModus(k: LangfristigeBestellungKosten) {
