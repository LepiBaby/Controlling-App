...
import type { PlanungsMonat } from '@/hooks/use-operativekosten-planung'

// PROJ-88: Massen-Anpassung für die langfristige Operativekosten-Planung. Funktional
// identisch zum Absatzplanungs-Dialog, aber auf eine einzige Wertart (Betrag in €)
// reduziert. Progressive Methoden gruppieren je Kategorie.
...
export interface OperativekostenBulkEditCell {
  kategorieId: string
  monat: PlanungsMonat
  currentValue: number
}

export interface OperativekostenBulkEditResult {
  kategorieId: string
  monat: PlanungsMonat
  newValue: number
}
...
interface Props {
  open: boolean
  onClose: () => void
  cells: OperativekostenBulkEditCell[]
  onApply: (results: OperativekostenBulkEditResult[]) => void
}
...
export function OperativekostenPlanungBulkEditDialog({ open, onClose, cells, onApply }: Props) {
