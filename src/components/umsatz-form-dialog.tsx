...
import { UmsatzTransaktion, UmsatzTransaktionInput } from '@/hooks/use-umsatz-transaktionen'

const TODAY = new Date().toISOString().split('T')[0]

interface UmsatzFormDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  transaktionToEdit: UmsatzTransaktion | null
  umsatzKategorien: KpiCategory[]
  salesPlattformen: KpiCategory[]
  produkte: KpiCategory[]
  onSave: (input: UmsatzTransaktionInput) => Promise<void>
}

export function UmsatzFormDialog({
  open,
  onOpenChange,
  transaktionToEdit,
  umsatzKategorien,
  ...
  const selectedKategorie = umsatzKategorien.find(c => c.id === kategorieId) ?? null
  const gruppen = umsatzKategorien.filter(c => c.level === 2 && c.parent_id === kategorieId)
  const untergruppen = umsatzKategorien.filter(c => c.level === 3 && c.parent_id === gruppeId)
  ...
  const handleSave = async () => {
    if (!kategorieId || !leistungsdatum || Number(betrag) <= 0) return
    setSaving(true)
    setSaveError(null)
    try {
      await onSave({
