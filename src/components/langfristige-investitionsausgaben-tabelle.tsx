...
// PROJ-92: Investitionsausgaben-Planung der Langfristigen Planung.
// Zeilenquelle = Investitionen-KPI-Modell der Version (Übergruppe → Untergruppe →
// Produkt). Auto-berechnet wird nur "Produktinvestitionen Einkauf"; alle anderen
// Kategorien sind rein manuell. KEINE Ist-/Vergangenheitsspalten — jede Monatsspalte
// ist eine Soll-Spalte. Bedien-/Anzeigemodell gespiegelt aus PROJ-91.
...
export function LangfristigeInvestitionsausgabenTabelle({ versionId }: { versionId: string }) {
  const {
    monate,
    kategorien,
    produkte,
  ...
  const { notizen, upsertNotiz, deleteNotiz } = useLangfristigePlanungNotizen(versionId, NOTIZEN_SEITE)
  ...
  const childrenByParent = useMemo(() => {
    const map = new Map<string, InvestKategorie[]>()
  ...
  const getSubgroups = useMemo(() => {
    return (l1: InvestKategorie): Array<{ id: string; name: string }> =>
      (childrenByParent.get(l1.id) ?? []).map(l2 => ({ id: l2.id, name: l2.name }))
  }, [childrenByParent])

  // Alle Übergruppen werden gezeigt (auch ohne Untergruppen → Header-only).
  const allExpandableIds = useMemo(() => {
    ...
    return ids
  }, [l1Kategorien, getSubgroups, produkte])
  ...
  if (l1Kategorien.length === 0) {
    return (
      <div className="rounded-md border p-8 text-center text-muted-foreground text-sm space-y-2">
        <p>Keine Investitionskategorien im KPI-Modell dieser Planversion vorhanden.</p>
        <p>
          Bitte die{' '}
          ...
          {' '}öffnen und Investitionen anlegen.
        </p>
      </div>
    )
  }
  ...
  return (
    <TooltipProvider delayDuration={400}>
      <div data-betrag-selektion="true" className="space-y-4" onMouseUp={() => { isDragging.current = false }}>
        ...
        {/* Hinweis: keine Produkte → keine Leaf-Zeilen pflegbar */}
        {produkte.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Keine Produkte im KPI-Modell dieser Planversion. Bitte in der{' '}
            <a href={`/dashboard/langfristige-planung/${versionId}/kpi-modell-verwaltung`} className="underline text-foreground">
              KPI-Modell Verwaltung
            </a>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                aus den Erstbestellungen dieser Version berechnet; alle anderen Felder werden leer.
