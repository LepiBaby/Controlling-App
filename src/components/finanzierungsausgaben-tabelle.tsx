...
import { useFinanzierungsausgaben, wertKey } from '@/hooks/use-finanzierungsausgaben'
import type { PlanungsWoche } from '@/hooks/use-finanzierungsausgaben'
...
export function FinanzierungsausgabenTabelle({ referenceDate }: { referenceDate?: Date } = {}) {
  const {
    ...
    upsertZelle,
    resetAll,
  } = useFinanzierungsausgaben(referenceDate)

  const { toast } = useToast()
  const { notizen, upsertNotiz, deleteNotiz, resetNotizen } = usePlanungNotizen('finanzierungs-ausgaben')
  ...
  // "Finanzierung" is a level-1 node; its direct children are level 2 (our "L1 rows")
  ...
  const flatRows = useMemo((): FlatRow[] => {
    ...
    rows.push({
      id: 'total',
      kind: 'total',
      label: 'Finanzierungsausgaben (Gesamt)',
      indent: 0,
  ...
  if (l1Kategorien.length === 0) {
    return (
      <div className="rounded-md border p-8 text-center text-muted-foreground text-sm space-y-2">
        <p>Keine Kategorien unterhalb des „Finanzierung"-Knotens vorhanden.</p>
        <p>
          Bitte im{' '}
          <a href="/dashboard/kpi-modell" className="underline text-foreground">KPI-Modell</a>
          {' '}Kategorien unterhalb von „Finanzierung" anlegen.
        </p>
      </div>
    )
  }
