...
import {
  useLangfristigeVerkaufsgebuehrEinstellungen,
  type LangfristigeVerkaufsgebuehrEinstellung,
} from '@/hooks/use-langfristige-verkaufsgebuehr-einstellungen'
...
function VerkaufsgebuehrEinstellungZeile({
  ...
  produkt: KpiCategory
  plattformId: string
  einstellung: LangfristigeVerkaufsgebuehrEinstellung
  onSave: (patch: Omit<LangfristigeVerkaufsgebuehrEinstellung, 'id'>) => Promise<void>
}) {
...
function PlattformTabelle({
  ...
  const { loading, error, getEinstellung, upsert, batchUpsert } =
    useLangfristigeVerkaufsgebuehrEinstellungen(versionId, plattformId)
    ...
    return (
      <div className="rounded-lg border bg-muted/30 p-8 text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Bitte zuerst in der KPI-Modell Verwaltung dieser Planversion Produkte anlegen, bevor
          Verkaufsgebühren gepflegt werden können.
        </p>
  ...
  return (
    <div className="space-y-3">
      <AlleGleichsetzenBereich plattformId={plattformId} produkte={produkte} onBatch={batchUpsert} />
...
export function LangfristigeVerkaufsgebuehrEinstellungenTabelle() {
  ...
  const sortedProdukte = useMemo(
    () => alleProdukte.filter(p => p.level === 1).sort((a, b) => a.sort_order - b.sort_order),
    [alleProdukte],
  )
  ...
  if (sortedPlattformen.length === 0) {
    return (
      <div className="rounded-lg border bg-muted/30 p-8 text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Bitte zuerst in der KPI-Modell Verwaltung dieser Planversion Sales-Plattformen anlegen,
          bevor Verkaufsgebühren gepflegt werden können.
        </p>
        <Link href={`/dashboard/langfristige-planung/${versionId}/kpi-modell-verwaltung`}>
          <Button variant="outline" size="sm" className="mt-2">
            Zur KPI-Modell Verwaltung
    ...
    <Tabs defaultValue={sortedPlattformen[0]?.id} className="space-y-4">
      ...
      {sortedPlattformen.map(p => (
        <TabsContent key={p.id} value={p.id} className="mt-0">
          <PlattformTabelle versionId={versionId} plattformId={p.id} produkte={sortedProdukte} />
        </TabsContent>
      ))}
    </Tabs>
  )
