...
// PROJ-98: Operative-Kosten-Matrix der Langfristigen Planung — L1-Gruppen (aufklappbar zu
// Untergruppen) + Gesamt-Zeile, sticky erste Spalte, drei Ansichtsmodi. Aufbau und Stil
// exakt wie die Umsatz-/Rentabilitätsauswertung-Matrix (PROJ-95/96).

// ─── Formatierung ──────────────────────────────────────────────────────────────

function formatBetrag(value: number): string {
  // -0 auf 0 normalisieren (Kostenwerte werden ×-1 gerechnet; -1 × 0 = -0 → „-0,00 €").
...
function formatProzentWert(value: number, basis: number): string {
  ...
  const norm = pct === 0 ? 0 : pct // -0 → 0 (siehe formatBetrag)
  return `${norm.toLocaleString('de-DE', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} %`
}
...
function flatten(nodes: OkNode[], expandedIds: Set<string>, indent: number, out: FlatRow[]) {
...
interface Props {
  model: OkModel
  anzeigemodus: OkAnzeigemodus
}

export function LangfristigeOperativeKostenAuswertungMatrix({ model, anzeigemodus }: Props) {
  const { columns, operativ, brutto, loading, error, isEmpty } = model
  const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set())

  const nodes = useMemo(() => computeCascade(operativ, columns), [operativ, columns])
  ...
  // ── Betragsselektion (analog PROJ-95/96) ──
      ...
      <div data-betrag-selektion="true" className="space-y-2">
        {isEmpty && (
          <p className="text-sm text-muted-foreground rounded-md border border-dashed px-4 py-2">
            Diese Planversion enthält noch keine operativen Kosten — alle Zeilen zeigen 0,00 €.
          </p>
        )}
