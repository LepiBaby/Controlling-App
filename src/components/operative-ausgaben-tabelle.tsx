...
import { useOperativeAusgaben, wertKey } from '@/hooks/use-operativeausgaben'
import type { PlanungsWoche } from '@/hooks/use-operativeausgaben'
...
interface FlatRow {
  ...
  label: string
  indent: number
  kategorieId?: string          // leaf: direct category ID for data lookup
  childLeafIds?: string[]       // header/total: IDs of all descendant leaf categories
  expandable: boolean
  expanded: boolean
}
...
export function OperativeAusgabenTabelle({ referenceDate }: { referenceDate?: Date } = {}) {
  const {
    ...
    upsertZelle,
    resetAll,
  } = useOperativeAusgaben(referenceDate)

  const { toast } = useToast()
  const { notizen, upsertNotiz, deleteNotiz, resetNotizen } = usePlanungNotizen('operative-ausgaben')
  ...
  // "Operativ" is a level-1 node; its direct children are level 2 (our "L1 rows")
  ...
  const flatRows = useMemo((): FlatRow[] => {
    ...
    rows.push({
      id: 'total',
      kind: 'total',
      label: 'Operative Ausgaben (Gesamt)',
      indent: 0,
  ...
  if (l1Kategorien.length === 0) {
    return (
      <div className="rounded-md border p-8 text-center text-muted-foreground text-sm space-y-2">
        <p>Keine Kategorien unterhalb des „Operativ"-Knotens vorhanden.</p>
        <p>
          Bitte im{' '}
          <a href="/dashboard/kpi-modell" className="underline text-foreground">KPI-Modell</a>
          {' '}Kategorien unterhalb von „Operativ" anlegen.
        </p>
      </div>
    )
  }
