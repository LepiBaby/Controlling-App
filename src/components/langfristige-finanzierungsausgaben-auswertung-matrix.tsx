...
// PROJ-100: Finanzierungsausgaben-Matrix der Langfristigen Planung — L1-Gruppen (aufklappbar zu
// Untergruppen) + Gesamt-Zeile, sticky erste Spalte, drei Ansichtsmodi. Aufbau und Stil
// exakt wie die Operative-Kosten-/Umsatz-/Rentabilitätsauswertung-Matrix (PROJ-95/96/98).
...
function flatten(nodes: FaNode[], expandedIds: Set<string>, indent: number, out: FlatRow[]) {
...
export function LangfristigeFinanzierungsausgabenAuswertungMatrix({ model, anzeigemodus }: Props) {
  const { columns, finanzierung, brutto, loading, error, isEmpty } = model
  ...
  const hatGruppen = useMemo(() => gruppenNodes(nodes).length > 0, [nodes])
  ...
  // ── Betragsselektion (analog PROJ-95/96/98) ──
  ...
  if (!hatGruppen) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed py-16 text-center text-muted-foreground">
        <BarChart2 className="h-8 w-8" />
        <p className="text-sm">
          Keine Finanzierungskategorien im KPI-Modell vorhanden. Bitte den Knoten „Finanzierung" im{' '}
          <Link href="/dashboard/kpi-modell" className="underline hover:text-foreground">KPI-Modell</Link> konfigurieren.
        </p>
      </div>
    )
  }
      ...
      <div data-betrag-selektion="true" className="space-y-2">
        {isEmpty && (
          <p className="text-sm text-muted-foreground rounded-md border border-dashed px-4 py-2">
            Diese Planversion enthält noch keine Finanzierungsausgaben — alle Zeilen zeigen 0,00 €.
          </p>
        )}
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {rows.map((row, idx) => {
                ...
                const isLeafDetail = row.kind === 'untergruppe'
