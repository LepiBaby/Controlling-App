...
function initDraft(t: EinnahmenTransaktion): EinnahmenDraft {
  return {
...
function validateDraft(d: EinnahmenDraft, kategorien: KpiCategory[]): Record<string, string> {
  const e: Record<string, string> = {}
  if (!d.zahlungsdatum) e.zahlungsdatum = 'Pflichtfeld'
...
function EinnahmenEditRow({
  ...
  t: EinnahmenTransaktion
  einnahmenKategorien: KpiCategory[]
  salesPlattformen: KpiCategory[]
  produkte: KpiCategory[]
  columnVisibility: ColumnVisibility
  onInlineUpdate: (id: string, input: Partial<EinnahmenTransaktionInput>) => Promise<void>
  onDelete: (id: string) => void
}) {
  const { toast } = useToast()
  const [draft, setDraft] = useState<EinnahmenDraft>(() => initDraft(t))
  ...
  const sel = einnahmenKategorien.find(c => c.id === draft.kategorieId) ?? null
  const gruppen = einnahmenKategorien.filter(c => c.level === 2 && c.parent_id === draft.kategorieId)
  const untergruppen = einnahmenKategorien.filter(c => c.level === 3 && c.parent_id === draft.gruppeId)
  ...
  const handleBlur = async () => {
    ...
    const newErrors = validateDraft(d, einnahmenKategorien)
    setErrors(newErrors)
    if (Object.keys(newErrors).length > 0) return

    const unchanged =
      d.zahlungsdatum === t.zahlungsdatum &&
  ...
  return (
    <TableRow className={cn('bg-muted/20', saving && 'opacity-60')}>
      <TableCell className="p-1">
        <input type="date" className={ic('zahlungsdatum')} value={draft.zahlungsdatum}
