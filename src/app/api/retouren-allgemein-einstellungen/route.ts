...
export async function GET() {
  ...
  const { data, error: dbErr } = await supabase
    ...
    .select('gruppierung, naechste_zahlung_basis_kw, naechste_zahlung_basis_jahr, zahlungsziel_tage')
    .eq('user_id', user!.id)
    .maybeSingle()
...
export async function PUT(request: Request) {
  ...
  const merged = {
    ...
    zahlungsziel_tage:
      'zahlungsziel_tage' in body ? (patch.zahlungsziel_tage ?? null) : (current?.zahlungsziel_tage ?? null),
