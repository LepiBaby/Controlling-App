...
const BASE_URL     = 'http://localhost/api/lagerausgaben-plattform-einstellungen'
...
describe('GET /api/lagerausgaben-plattform-einstellungen', () => {
...
describe('PUT /api/lagerausgaben-plattform-einstellungen', () => {
  ...
  it('returns 200 updating gruppierung only (zahlungsziel preserved)', async () => {
    mockSelectAndUpsert(
      { gruppierung: 'woechentlich', zahlungsziel_tage: 14, naechste_zahlung_basis_kw: null, naechste_zahlung_basis_jahr: null },
      { gruppierung: 'monatlich', zahlungsziel_tage: 14, naechste_zahlung_basis_kw: null, naechste_zahlung_basis_jahr: null }
    )
  ...
  it('returns 200 setting KW/Jahr basis', async () => {
    mockSelectAndUpsert(
      { gruppierung: 'monatlich', zahlungsziel_tage: null, naechste_zahlung_basis_kw: null, naechste_zahlung_basis_jahr: null },
      { ...MOCK_EINSTELLUNGEN }
    )
    const res = await PUT(req(BASE_URL, {
      method: 'PUT',
  ...
  it('returns 200 clearing zahlungsziel (null)', async () => {
    mockSelectAndUpsert(
      { gruppierung: 'monatlich', zahlungsziel_tage: 30, naechste_zahlung_basis_kw: null, naechste_zahlung_basis_jahr: null },
      { gruppierung: 'monatlich', zahlungsziel_tage: null, naechste_zahlung_basis_kw: null, naechste_zahlung_basis_jahr: null }
    )
    const res = await PUT(req(BASE_URL, {
      method: 'PUT',
  ...
  it('returns 200 on first-time upsert (no current row)', async () => {
    mockSelectAndUpsert(
      null,
      { gruppierung: 'quartalsweise', zahlungsziel_tage: null, naechste_zahlung_basis_kw: null, naechste_zahlung_basis_jahr: null }
    )
    const res = await PUT(req(BASE_URL, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
