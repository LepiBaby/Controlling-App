...
describe('GET /api/bestellplanung/bestellungen/[id]', () => {
  ...
  it('returns enriched bestellung on happy path', async () => {
    ...
    expect(body.herkunft).toBe('algorithmus')
    expect(body.produkte).toEqual([])
  })

  it('returns herkunft=manuell for manually created bestellung', async () => {
    const manualRow = { ...BASE_ROW, herkunft: 'manuell' }
    mockFrom
      .mockReturnValueOnce(ch({ data: manualRow, error: null }))
...
describe('PUT /api/bestellplanung/bestellungen/[id]', () => {
  ...
  it('updates status and returns enriched bestellung', async () => {
    ...
    mockFrom
      .mockReturnValueOnce(OK_NULL) // bestellungen_konsolidierungsmitglieder group check → not in group
      ...
      .mockReturnValueOnce(EMPTY) // enrich: bestellungen_konsolidierungsmitglieder
      .mockReturnValueOnce(EMPTY) // kpi_categories names
  ...
  it('returns 409 when changing status of consolidated bestellung', async () => {
    mockFrom.mockReturnValueOnce(ch({ data: { gruppe_id: 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11' }, error: null }))
    const res = await PUT(req('http://localhost/api/bestellplanung/bestellungen/b-1', {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'laufend' }),
    }), makeParams('b-1'))
    ...
    const body = await res.json()
    expect(body.error).toBe('in_gruppe')
  })

  it('returns 500 when db update fails', async () => {
    mockFrom
      .mockReturnValueOnce(OK_NULL) // bestellungen_konsolidierungsmitglieder group check → not in group
      .mockReturnValueOnce(ch({ data: null, error: { message: 'Update failed' } }))
    const res = await PUT(req('http://localhost/api/bestellplanung/bestellungen/b-1', {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'laufend' }),
    }), makeParams('b-1'))
