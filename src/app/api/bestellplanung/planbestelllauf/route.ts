...
function calcTagesdurchschnitt(
  ...
  if (e.berechnungsart.startsWith('gewichtet_')) {
    ...
    if (w1 == null || w2 == null || w3 == null) return (s1 + s2 + s3) / periodDays
    return (w1 * (s1 / third) + w2 * (s2 / third) + w3 * (s3 / third)) / 100
  }
...
export async function POST() {
  ...
  const [
    ...
    supabase.from('absatz_planung').select('produkt_id, kw_year, kw_number, absatz_manuell').eq('user_id', user!.id).not('sku_id', 'is', null).not('absatz_manuell', 'is', null).limit(20000),
    supabase.from('bestellungen').select('id, status, bestelldatum, ankunftsdatum').eq('user_id', user!.id).in('status', ['plan', 'laufend']).limit(500),
  ])
  ...
  // ─── 5. Historical avg absatz per product (daily → weekly) ───────────────────
  const einstellungen: AbsatzEinstellung[] = einstellungenRes.data ?? []
  const avgWochenabsatzByProdukt = new Map<string, number>()
  ...
  if (einstellungen.length > 0 && allSkuIds.length > 0) {
    ...
    for (const e of einstellungen) {
      ...
      for (const skuId of skuIds) {
        ...
        avgWochenabsatzByProdukt.set(e.produkt_id, (avgWochenabsatzByProdukt.get(e.produkt_id) ?? 0) + td)
      }
    }

    // Convert daily to weekly
    for (const [k, v] of avgWochenabsatzByProdukt) {
      avgWochenabsatzByProdukt.set(k, v * 7)
    }
  }

  // ─── 6. Absatz planning totals per product per KW ────────────────────────────
  const absatzPlanungGrouped = new Map<string, number>() // key: "produktId:year:week"
  for (const row of (absatzPlanungRes.data ?? []) as Array<{ produkt_id: string; kw_year: number; kw_number: number; absatz_manuell: number }>) {
    const key = `${row.produkt_id}:${row.kw_year}:${row.kw_number}`
    absatzPlanungGrouped.set(key, (absatzPlanungGrouped.get(key) ?? 0) + (row.absatz_manuell ?? 0))
  }

  const absatzplanung = Array.from(absatzPlanungGrouped.entries()).map(([key, menge]) => {
    const [produkt_id, yearStr, weekStr] = key.split(':')
    return { produkt_id, kw_year: Number(yearStr), kw_number: Number(weekStr), menge }
  })
  ...
  if (existingBestellungIds.length > 0) {
    const [prodRes, skuMengenBestRes] = await Promise.all([
      ...
      supabase.from('bestellungen_sku_mengen').select('bestellung_id, sku_id, menge_praktisch').in('bestellung_id', existingBestellungIds),
    ])
    ...
    const skuByBest = new Map<string, Array<{ sku_id: string; produkt_id: string; menge_praktisch: number }>>()
    const skuParentMap = new Map(skusData.map(s => [s.id, s.parent_id]))
    for (const sm of (skuMengenBestRes.data ?? []) as Array<{ bestellung_id: string; sku_id: string; menge_praktisch: number }>) {
    ...
    bestehendeBestellungen = (bestellungenRes.data ?? []).map((b: { id: string; status: string; bestelldatum: string | null; ankunftsdatum: string | null }) => ({
      bestellung_id: b.id,
  ...
  const produkte: ProduktInput[] = produktIds.map(pid => {
    ...
    return {
      ...
      avg_wochenabsatz: avgWochenabsatzByProdukt.get(pid) ?? 0,
    }
