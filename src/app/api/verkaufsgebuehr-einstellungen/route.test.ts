...
describe('GET /api/verkaufsgebuehr-einstellungen', () => {
  it('returns 400 when plattform_id is missing', async () => {
    const res = await GET(req('http://localhost/api/verkaufsgebuehr-einstellungen'))
  ...
  it('returns 400 when plattform_id is not a valid UUID', async () => {
    const res = await GET(req('http://localhost/api/verkaufsgebuehr-einstellungen?plattform_id=not-a-uuid'))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/plattform_id/)
  })
  ...
  it('returns 200 with empty array when no einstellungen exist', async () => {
    ...
    const res = await GET(req(`http://localhost/api/verkaufsgebuehr-einstellungen?plattform_id=${PLATTFORM_ID}`))
  ...
  it('returns 200 with einstellungen for the given plattform', async () => {
    ...
    expect(body[0].verkaufsgebuehr_prozent).toBe(15.5)
...
describe('PUT /api/verkaufsgebuehr-einstellungen', () => {
  ...
  it('returns 200 on upsert with a positive decimal value', async () => {
    ...
    const res = await PUT(req('http://localhost/api/verkaufsgebuehr-einstellungen', {
      method: 'PUT',
    ...
    expect(body.verkaufsgebuehr_prozent).toBe(15.5)
  })

  it('returns 200 on upsert with null (clearing the value)', async () => {
    upsertMock({ ...MOCK_EINSTELLUNG, verkaufsgebuehr_prozent: null })
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.verkaufsgebuehr_prozent).toBeNull()
  })

  it('returns 200 on upsert without verkaufsgebuehr_prozent field (treated as null)', async () => {
    upsertMock({ ...MOCK_EINSTELLUNG, verkaufsgebuehr_prozent: null })
  ...
  it('returns 200 on upsert with value over 100 (no upper limit)', async () => {
    upsertMock({ ...MOCK_EINSTELLUNG, verkaufsgebuehr_prozent: 120 })
    ...
    const res = await PUT(req('http://localhost/api/verkaufsgebuehr-einstellungen', {
      ...
      body: JSON.stringify({
        sales_plattform_id: PLATTFORM_ID,
        produkt_id: PRODUKT_ID,
        verkaufsgebuehr_prozent: 120,
      }),
    }))
    expect(res.status).toBe(200)
  })

  it('returns 400 when verkaufsgebuehr_prozent is negative', async () => {
    const res = await PUT(req('http://localhost/api/verkaufsgebuehr-einstellungen', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
