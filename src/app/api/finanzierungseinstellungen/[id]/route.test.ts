...
describe('PUT /api/finanzierungseinstellungen/[id]', () => {
  it('updates eintrag and returns it with kategorie_name and computed brutto', async () => {
    ...
    const res = await PUT(
      req(`http://localhost/api/finanzierungseinstellungen/${EINTRAG_ID}`, {
        method: 'PUT',
    ...
    expect(body.name).toBe('Bankkredit aktualisiert')
    expect(body.kategorie_name).toBe('Kredite')
...
describe('DELETE /api/finanzierungseinstellungen/[id]', () => {
  it('deletes eintrag and returns 204', async () => {
    ...
    const res = await DELETE(
      req(`http://localhost/api/finanzierungseinstellungen/${EINTRAG_ID}`, { method: 'DELETE' }),
      makeCtx(EINTRAG_ID),
    )
