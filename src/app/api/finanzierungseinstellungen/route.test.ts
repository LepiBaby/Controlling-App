...
const MOCK_DB_ROW = {
  ...
  name: 'Bankkredit Betriebsmittel',
...
describe('GET /api/finanzierungseinstellungen', () => {
  it('returns 200 with mapped eintraege (kategorie_name flattened)', async () => {
    ...
    expect(body[0].kategorie_name).toBe('Kredite')
...
describe('POST /api/finanzierungseinstellungen', () => {
  it('creates eintrag with monatlich frequency and 0% USt', async () => {
    mockFrom.mockReturnValueOnce({
      insert: () => ({
        select: () => ({
    ...
    const res = await POST(req('http://localhost/api/finanzierungseinstellungen', {
      method: 'POST',
