...
describe('GET /api/einnahmen-transaktionen', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    // Default mock: returns empty list with count=0 and empty sum
  ...
  it('returns 200 with data/total/totalBetrag shape', async () => {
    ...
    const res = await GET(req('http://localhost/api/einnahmen-transaktionen'))
...
describe('POST /api/einnahmen-transaktionen', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockFrom.mockReturnValue({
      insert: () => ({
  ...
  it('returns 201 with valid payload', async () => {
    const res = await POST(req('http://localhost/api/einnahmen-transaktionen', {
      method: 'POST',
  ...
  it('returns 400 for missing zahlungsdatum', async () => {
    const res = await POST(req('http://localhost/api/einnahmen-transaktionen', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 400 for missing betrag', async () => {
    const res = await POST(req('http://localhost/api/einnahmen-transaktionen', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ zahlungsdatum: '2024-01-15', kategorie_id: '00000000-0000-0000-0000-000000000001' }),
    }))
    expect(res.status).toBe(400)
  })
  ...
  it('returns 400 for invalid date format', async () => {
    const res = await POST(req('http://localhost/api/einnahmen-transaktionen', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...VALID_TRANSAKTION, zahlungsdatum: '15.01.2024' }),
    }))
    expect(res.status).toBe(400)
  })
  ...
  it('returns 400 for missing kategorie_id', async () => {
    const res = await POST(req('http://localhost/api/einnahmen-transaktionen', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ zahlungsdatum: '2024-01-15', betrag: 100 }),
    }))
    expect(res.status).toBe(400)
  })
