...
export async function DELETE(req?: Request) {
  ...
  if (abYear && abWeek) {
    ...
    const { error: e1 } = await supabase.from('finanzierungs_planung').delete()
      .eq('user_id', user!.id).gt('kw_year', yr)
    const { error: e2 } = await supabase.from('finanzierungs_planung').delete()
      .eq('user_id', user!.id).eq('kw_year', yr).gte('kw_number', wk)
    if (e1 || e2) return NextResponse.json({ error: (e1 ?? e2)!.message }, { status: 500 })
  } else {
    const { error: dbErr } = await supabase.from('finanzierungs_planung').delete().eq('user_id', user!.id)
    if (dbErr) return NextResponse.json({ error: dbErr.message }, { status: 500 })
  }
