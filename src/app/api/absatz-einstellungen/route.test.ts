...
describe('GET /api/absatz-einstellungen', () => {
  it('returns 400 when plattform_id is missing', async () => {
    const res = await GET(req('http://localhost/api/absatz-einstellungen'))
  ...
  it('returns 400 when plattform_id is not a valid UUID', async () => {
    const res = await GET(req('http://localhost/api/absatz-einstellungen?plattform_id=not-a-uuid'))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/plattform_id/)
  })
  ...
  it('returns 200 with empty array when no einstellungen exist', async () => {
    ...
    const res = await GET(req(`http://localhost/api/absatz-einstellungen?plattform_id=${PLATTFORM_ID}`))
...
describe('PUT /api/absatz-einstellungen', () => {
  ...
  it('returns 200 on upsert with non-gewichtet berechnungsart', async () => {
    ...
    const res = await PUT(req('http://localhost/api/absatz-einstellungen', {
      method: 'PUT',
