...
// PROJ-88: Versionsgebundene Operativekosten-Planung — Betrag je Zellkoordinate
// (globale Operativ-Leaf-Kategorie × Monat). Keine historische Vorbelegung; alle
// Werte werden manuell gepflegt. Die Kategoriestruktur stammt aus dem globalen
// KPI-Modell, die Werte sind pro Planversion + Nutzer isoliert.
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const { data, error: dbErr } = await supabase
    .from('langfristige_operativekosten_planung')
    .select(SELECT_COLS)
    .eq('user_id', user!.id)
