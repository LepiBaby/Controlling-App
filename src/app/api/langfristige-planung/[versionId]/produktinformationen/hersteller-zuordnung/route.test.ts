...
function ctx() {
  return { params: Promise.resolve({ versionId: VERSION_ID }) }
}
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/produktinformationen/hersteller-zuordnung`
...
describe('hersteller-zuordnung', () => {
  it('GET returns 200', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: [{ id: 'z1', produkt_id: PRODUKT_ID, hersteller_id: null }], error: null }))
  ...
  it('PUT upserts (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { id: 'z1', produkt_id: PRODUKT_ID, hersteller_id: null }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, hersteller_id: null })).status).toBe(200)
  })

  it('PUT returns 400 on invalid body', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    expect((await put({ produkt_id: 'nope', hersteller_id: null })).status).toBe(400)
  })

  it('PUT returns 401 when unauthenticated', async () => {
    await unauth()
    expect((await put({ produkt_id: PRODUKT_ID, hersteller_id: null })).status).toBe(401)
  })
})
