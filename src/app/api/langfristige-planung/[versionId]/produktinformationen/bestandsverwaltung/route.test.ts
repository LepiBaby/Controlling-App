...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/produktinformationen/bestandsverwaltung`
...
describe('bestandsverwaltung', () => {
  ...
  it('PUT upserts (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { id: 'b1', produkt_id: PRODUKT_ID, sicherheitsbestand: 3, zielreichweite_wochen: 6 }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, sicherheitsbestand: 3, zielreichweite_wochen: 6 })).status).toBe(200)
  })

  it('PUT returns 400 on negative sicherheitsbestand', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, sicherheitsbestand: -1 })).status).toBe(400)
  })

  it('PUT returns 401 when unauthenticated', async () => {
    await unauth()
    expect((await put({ produkt_id: PRODUKT_ID, sicherheitsbestand: 3 })).status).toBe(401)
  })
})
