...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/produktinformationen/containerkapazitaet`
...
describe('containerkapazitaet', () => {
  ...
  it('PUT upserts (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { id: 'k1', produkt_id: PRODUKT_ID, laenge_cm: 10, breite_cm: 5, hoehe_cm: 3 }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, laenge_cm: 10, breite_cm: 5, hoehe_cm: 3 })).status).toBe(200)
  })

  it('PUT returns 400 on invalid produkt_id', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    expect((await put({ produkt_id: 'nope', laenge_cm: 10 })).status).toBe(400)
  })

  it('PUT returns 401 when unauthenticated', async () => {
    await unauth()
    expect((await put({ produkt_id: PRODUKT_ID, laenge_cm: 10 })).status).toBe(401)
  })
})
