...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/produktinformationen/moq`
...
describe('moq', () => {
  it('GET returns 200', async () => {
    ...
    mockFrom.mockReturnValueOnce(chain({ data: [{ id: 'm1', produkt_id: PRODUKT_ID, ebene: 'produkt', moq: 10 }], error: null }))
  ...
  it('PUT upserts (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { id: 'm1', produkt_id: PRODUKT_ID, ebene: 'produkt', moq: 10 }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, ebene: 'produkt', moq: 10 })).status).toBe(200)
  })

  it('PUT returns 400 on moq < 1', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, moq: 0 })).status).toBe(400)
  })

  it('PUT returns 401 when unauthenticated', async () => {
    await unauth()
    expect((await put({ produkt_id: PRODUKT_ID, moq: 10 })).status).toBe(401)
  })
})
