...
// PROJ-77: MOQ je Produkt (nur Produktebene — Langfristig hat keine SKUs).

const TABLE = 'langfristige_produktinformationen_moq'

const UpsertSchema = z.object({
  produkt_id: z.string().uuid(),
  ebene: z.enum(['produkt', 'sku']).default('produkt'),
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const { data, error: dbErr } = await supabase
    .from(TABLE)
    .select('id, produkt_id, ebene, moq')
    .eq('user_id', user!.id)
