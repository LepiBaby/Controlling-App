...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/produktinformationen/produktkosten`
...
describe('produktkosten', () => {
  ...
  it('PUT upserts (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { id: 'p1', produkt_id: PRODUKT_ID, warenkosten: 5, zollsatz_pct: 3 }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, warenkosten: 5, zollsatz_pct: 3 })).status).toBe(200)
  })

  it('PUT returns 400 on zollsatz > 100', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    expect((await put({ produkt_id: PRODUKT_ID, zollsatz_pct: 200 })).status).toBe(400)
  })

  it('PUT returns 401 when unauthenticated', async () => {
    await unauth()
    expect((await put({ produkt_id: PRODUKT_ID, warenkosten: 5 })).status).toBe(401)
  })
})
