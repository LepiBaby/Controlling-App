...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/produktinformationen/container-global`
...
describe('container-global', () => {
  ...
  it('PUT upserts (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { id: 'cg1', volumen_20dc: 33, volumen_40hq: 76 }, error: null }))
    expect((await put({ volumen_20dc: 33, volumen_40hq: 76 })).status).toBe(200)
  })

  it('PUT returns 400 on negative volume', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    expect((await put({ volumen_20dc: -1 })).status).toBe(400)
  })

  it('PUT returns 401 when unauthenticated', async () => {
    await unauth()
    expect((await put({ volumen_20dc: 33 })).status).toBe(401)
  })
})
