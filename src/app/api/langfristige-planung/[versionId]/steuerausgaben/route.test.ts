...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/steuerausgaben`
...
function put(body: unknown, id = VERSION_ID) {
  return PUT(
    new Request(`http://localhost/api/langfristige-planung/${id}/steuerausgaben`, {
      method: 'PUT',
...
describe('GET /api/langfristige-planung/[versionId]/steuerausgaben', () => {
...
describe('PUT /api/langfristige-planung/[versionId]/steuerausgaben', () => {
  ...
  it('accepts a NEGATIVE betrag (USt-Erstattung) (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: { ...CELL, betrag_manuell: -250.75 }, error: null }))
    const res = await put({ ...CELL, betrag_manuell: -250.75 })
    expect(res.status).toBe(200)
  })
  ...
  it('deletes the cell when betrag_manuell is null (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null })) // version check
  ...
  it('returns 400 on invalid body (missing kategorie_id)', async () => {
    const res = await put({ jahr: 2026, monat: 4, betrag_manuell: 100 })
    expect(res.status).toBe(400)
  })
  ...
  it('returns 404 for foreign version (valid body)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: null, error: null }))
    const res = await put(CELL)
    expect(res.status).toBe(404)
  })
...
describe('DELETE /api/langfristige-planung/[versionId]/steuerausgaben', () => {
  it('clears all manual values + notes (200)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null })) // version check
  ...
  it('returns 500 when value-delete fails', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
