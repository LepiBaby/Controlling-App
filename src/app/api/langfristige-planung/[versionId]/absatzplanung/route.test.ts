...
const URL_BASE = `http://localhost/api/langfristige-planung/${VERSION_ID}/absatzplanung`
...
function put(body: unknown, id = VERSION_ID) {
  return PUT(
    new Request(`http://localhost/api/langfristige-planung/${id}/absatzplanung`, {
      method: 'PUT',
...
describe('GET /api/langfristige-planung/[versionId]/absatzplanung', () => {
...
describe('PUT /api/langfristige-planung/[versionId]/absatzplanung', () => {
  ...
  it('accepts null values (clearing a cell)', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    mockFrom.mockReturnValueOnce(chain({ data: [], error: null }))
    const res = await put({ ...CELL, absatz: null, effektiver_vk: null })
    expect(res.status).toBe(200)
  })
  ...
  it('returns 400 on negative absatz', async () => {
    mockFrom.mockReturnValueOnce(chain({ data: { id: VERSION_ID }, error: null }))
    const res = await put({ ...CELL, absatz: -5 })
    expect(res.status).toBe(400)
  })
