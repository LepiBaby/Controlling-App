...
describe('PATCH /api/umsatz-transaktionen/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 200 when updating betrag', async () => {
    const res = await PATCH(
      req('http://localhost/api/umsatz-transaktionen/txn-1', {
        method: 'PATCH',
  ...
  it('returns 200 when updating leistungsdatum', async () => {
    const res = await PATCH(
      req('http://localhost/api/umsatz-transaktionen/txn-1', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ leistungsdatum: '2024-02-01' }),
      }),
      ROUTE_CTX,
    )
    expect(res.status).toBe(200)
  ...
  it('returns 400 for invalid date format', async () => {
    const res = await PATCH(
      req('http://localhost/api/umsatz-transaktionen/txn-1', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ leistungsdatum: '15.01.2024' }),
      }),
      ROUTE_CTX,
    )
    expect(res.status).toBe(400)
...
describe('DELETE /api/umsatz-transaktionen/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockFrom.mockReturnValue({
      delete: () => ({
  ...
  it('returns 204 on successful delete', async () => {
    const res = await DELETE(
      req('http://localhost/api/umsatz-transaktionen/txn-1', { method: 'DELETE' }),
      ROUTE_CTX,
    )
