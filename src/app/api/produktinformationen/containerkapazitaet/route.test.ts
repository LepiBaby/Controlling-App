...
const MOCK_ENTRY = { id: EINTRAG_ID, produkt_id: PRODUKT_ID, laenge_cm: 30, breite_cm: 20, hoehe_cm: 15 }
...
describe('GET /api/produktinformationen/containerkapazitaet', () => {
  it('returns 200 with dimensions list', async () => {
    ...
    expect(body[0].laenge_cm).toBe(30)
  })
...
describe('PUT /api/produktinformationen/containerkapazitaet', () => {
  ...
  it('returns 200 on successful upsert', async () => {
    ...
    const res = await PUT(req('http://localhost/', {
      ...
      body: JSON.stringify({ produkt_id: PRODUKT_ID, laenge_cm: 30, breite_cm: 20, hoehe_cm: 15 }),
    }))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.laenge_cm).toBe(30)
  })
  ...
  it('returns 400 when produkt_id is missing', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ laenge_cm: 30 }),
    }))
  ...
  it('returns 400 when produkt_id is not a valid UUID', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: 'not-a-uuid', laenge_cm: 30 }),
    }))
    expect(res.status).toBe(400)
  })
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: PRODUKT_ID, laenge_cm: 30 }),
    }))
    expect(res.status).toBe(401)
  })
