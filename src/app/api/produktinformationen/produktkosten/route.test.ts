...
const MOCK_ENTRY = { id: EINTRAG_ID, produkt_id: PRODUKT_ID, warenkosten: 12.50, zollsatz_pct: 5.5 }
...
describe('GET /api/produktinformationen/produktkosten', () => {
  it('returns 200 with produktkosten list', async () => {
    ...
    expect(body[0].warenkosten).toBe(12.50)
  })
...
describe('PUT /api/produktinformationen/produktkosten', () => {
  ...
  it('returns 200 on successful upsert', async () => {
    ...
    const res = await PUT(req('http://localhost/', {
      ...
      body: JSON.stringify({ produkt_id: PRODUKT_ID, warenkosten: 12.50, zollsatz_pct: 5.5 }),
    }))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.zollsatz_pct).toBe(5.5)
  })
  ...
  it('returns 400 when produkt_id is missing', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ warenkosten: 12.50 }),
    }))
    expect(res.status).toBe(400)
  })

  it('returns 400 when zollsatz exceeds 100', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: PRODUKT_ID, zollsatz_pct: 110 }),
    }))
    expect(res.status).toBe(400)
  })
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: PRODUKT_ID, warenkosten: 12.50 }),
    }))
    expect(res.status).toBe(401)
  })
