...
describe('GET /api/produktinformationen/bestandsverwaltung', () => {
  it('returns 200 with bestandsverwaltung list', async () => {
    ...
    expect(body[0].sicherheitsbestand).toBe(100)
  })
...
describe('PUT /api/produktinformationen/bestandsverwaltung', () => {
  ...
  it('returns 400 when produkt_id is missing', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sicherheitsbestand: 100 }),
    }))
  ...
  it('returns 400 when produkt_id is not a valid UUID', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: 'not-a-uuid', sicherheitsbestand: 100 }),
    }))
    expect(res.status).toBe(400)
  })
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: PRODUKT_ID, sicherheitsbestand: 100 }),
    }))
    expect(res.status).toBe(401)
  })
