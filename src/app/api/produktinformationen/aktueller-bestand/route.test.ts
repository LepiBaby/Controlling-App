...
const MOCK_ENTRY = { id: EINTRAG_ID, produkt_id: PRODUKT_ID, bestand: 250 }
...
describe('GET /api/produktinformationen/aktueller-bestand', () => {
  it('returns 200 with bestand list', async () => {
    ...
    expect(body[0].bestand).toBe(250)
  })
...
describe('PUT /api/produktinformationen/aktueller-bestand', () => {
  ...
  it('returns 200 on successful upsert', async () => {
    ...
    const res = await PUT(req('http://localhost/', {
      ...
      body: JSON.stringify({ produkt_id: PRODUKT_ID, bestand: 250 }),
    }))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.bestand).toBe(250)
  })
  ...
  it('returns 400 when produkt_id is missing', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ bestand: 250 }),
    }))
  ...
  it('returns 400 when produkt_id is not a valid UUID', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: 'not-a-uuid', bestand: 250 }),
    }))
    expect(res.status).toBe(400)
  })

  it('returns 400 when bestand is negative', async () => {
    const res = await PUT(req('http://localhost/', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ produkt_id: PRODUKT_ID, bestand: -5 }),
    }))
    expect(res.status).toBe(400)
  })
