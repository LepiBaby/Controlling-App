...
describe('GET /api/steuerausgaben-planung', () => {
  ...
  it('returns 200 with existing entries', async () => {
    ...
    expect(body[0].betrag_manuell).toBe(1500.00)
    expect(body[0].ist_berechnet).toBe(false)
  })
...
describe('PUT /api/steuerausgaben-planung', () => {
  function req(body: unknown) {
    return new Request('http://localhost/api/steuerausgaben-planung', {
      method: 'PUT',
  ...
  it('upserts an entry and returns 200', async () => {
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.betrag_manuell).toBe(1500.00)
  })

  it('allows negative betrag_manuell (UST Erstattung)', async () => {
    mockFrom.mockReturnValueOnce({
      upsert: () => ({
        select: () => ({ single: () => ({ data: { ...MOCK_EINTRAG, betrag_manuell: -250 }, error: null }) }),
      }),
    })
    ...
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.betrag_manuell).toBe(-250)
  })
...
describe('DELETE /api/steuerausgaben-planung', () => {
  function buildDelete(query = '') {
    return new Request(`http://localhost/api/steuerausgaben-planung${query ? '?' + query : ''}`, {
      method: 'DELETE',
    })
  }

  it('deletes all manual entries when no params given', async () => {
    mockFrom.mockReturnValueOnce({
      delete: () => ({
        eq: () => ({ eq: () => ({ error: null }) }),
      }),
  ...
  it('deletes future manual entries from given KW onwards', async () => {
    const eqChain = {
      gt: () => ({ error: null }),
      gte: () => ({ error: null }),
      eq: () => ({ gte: () => ({ error: null }) }),
    }
    mockFrom.mockReturnValueOnce({ delete: () => ({ eq: () => ({ eq: () => eqChain }) }) })
    mockFrom.mockReturnValueOnce({ delete: () => ({ eq: () => ({ eq: () => eqChain }) }) })
  ...
  it('returns 500 when db delete fails', async () => {
    mockFrom.mockReturnValueOnce({
      delete: () => ({ eq: () => ({ eq: () => ({ error: { message: 'DB error' } }) }) }),
    })
