...
// DELETE only removes manual entries (ist_berechnet = false).
// With ab_kw_year + ab_kw_number: removes manual entries from that KW onwards.
// Without params: removes all manual entries (global reset).
export async function DELETE(req: Request) {
  ...
  if (abYear && abWeek) {
    ...
    const { error: e1 } = await supabase.from('steuerausgaben_planung').delete()
      .eq('user_id', user!.id).eq('ist_berechnet', false).gt('kw_year', yr)
    const { error: e2 } = await supabase.from('steuerausgaben_planung').delete()
      .eq('user_id', user!.id).eq('ist_berechnet', false).eq('kw_year', yr).gte('kw_number', wk)
