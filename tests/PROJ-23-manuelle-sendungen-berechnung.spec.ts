...
test('login page still renders correctly after PROJ-23 changes', async ({ page }) => {
...
test('dashboard page still redirects unauthenticated after PROJ-23 (regression)', async ({ page }) => {
  await page.goto('/dashboard')
  await expect(page).toHaveURL(/\/login/)
})

test('bestandsverwaltung page still redirects unauthenticated after PROJ-23 (regression)', async ({ page }) => {
  await page.goto('/dashboard/bestandsverwaltung')
  await expect(page).toHaveURL(/\/login/)
})

test('produktkosten page still redirects unauthenticated after PROJ-23 (regression)', async ({ page }) => {
  await page.goto('/dashboard/produktkosten')
  await expect(page).toHaveURL(/\/login/)
})

test('rentabilitaet auswertung still redirects unauthenticated after PROJ-23 (regression)', async ({ page }) => {
  await page.goto('/dashboard/rentabilitaet')
  await expect(page).toHaveURL(/\/login/)
})

test('kpi-modell still redirects unauthenticated after PROJ-23 (regression)', async ({ page }) => {
