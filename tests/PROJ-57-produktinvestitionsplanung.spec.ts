...
// Hook-Tests (Produktinvestitionen-Filter, isNewWeek, upsertZelle, Rollback) sind durch Vitest abgedeckt (29 Tests).

// ─── Seitenexistenz (kein 404) ───────────────────────────────────────────────

test('/dashboard/kurzfristige-planung/produktinvestitionsplanung liefert keinen 404-Fehler', async ({ page }) => {
  const response = await page.goto('/dashboard/kurzfristige-planung/produktinvestitionsplanung')
  expect(response?.status()).toBeLessThan(400)
})

// ─── Auth-Guard ──────────────────────────────────────────────────────────────

test('unauthenticated user is redirected from /dashboard/kurzfristige-planung/produktinvestitionsplanung to /login', async ({ page }) => {
...
// ─── Regression: Operative Planung weiterhin erreichbar ──────────────────────
...
// Navigation & Einstieg:
//   ✅ Linke Navigation im Bereich "Kurzfristige Planung" zeigt Eintrag "Produktinvestitionsplanung"
//   ✅ Kachel "Produktinvestitionsplanung" auf /dashboard/kurzfristige-planung im Abschnitt "Planung" vorhanden
...
//   ✅ Spaltenheader zeigt "Produktinvestition" als Zeilenbeschriftung
//   ✅ Zeitraum-Label oben links (z.B. "KW25 / 2026 – KW37 / 2026")
//
// Zeilenhierarchie:
//   ✅ Ganz oben: Gesamt-Zeile "Produktinvestitionen (Gesamt)" — nicht editierbar, aggregiert alle Leaf-Zeilen
...
//   ✅ Nur Kategorien unter dem "Produktinvestitionen"-Knoten im KPI-Modell werden angezeigt
//   ✅ Kategorien außerhalb des Produktinvestitionen-Teilbaums (z.B. "Marketing") sind nicht sichtbar
...
//
// Leerer Zustand:
//   ✅ Wenn kein "Produktinvestitionen"-Knoten im KPI-Modell: Hinweistext + Link zu KPI-Modell-Einstellungen
//
...
//   ✅ kontext = 'produktinvestitions_planung' (separate Notizen von anderen Planungsseiten)
//
